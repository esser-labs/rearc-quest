...
resource "aws_s3_object" "app_zip" {
  bucket = aws_s3_bucket.jed_rearc_quest.id
  key    = "${var.last_run_commit}.zip"
  source = "${path.module}/${var.last_run_commit}.zip"
}
...
resource "aws_elastic_beanstalk_application" "jed_rearc_quest_node" {
  name        = "${var.elastic_name}-node"
  description = "${var.elastic_name}-node"
}

resource "aws_elastic_beanstalk_environment" "jed_rearc_quest_node" {
  name         = "${var.elastic_name}-node"
  application  = aws_elastic_beanstalk_application.jed_rearc_quest_node.name
  cname_prefix = "rearc-quest-node"
...
resource "aws_elastic_beanstalk_application_version" "jed_rearc_quest_node" {
  name        = "${var.elastic_name}-${var.last_run_commit}"
  application = aws_elastic_beanstalk_application.jed_rearc_quest_node.name
  description = "application version created by terraform"
  bucket      = aws_s3_bucket.jed_rearc_quest.id
