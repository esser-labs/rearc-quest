...
Also, if I was running SSL in production I would terminate SSL at the CDN for my public domain. Then I would have a private domain where I would terminate the SSL for CDN backend look ups at the ELB.
I had issues where when I launched this application into EKS it did not think it was running AWS or in Docker. As a result I had to add an Elastic Beanstalk deployment to the Terraform in order to run it and get the secret word. I had assumed running the container in EKS with a Kubernetes services would take care of steps 1-4 in one shot. Obviously it created the ELB but the binaries did not work as I imagined they would in EKS.
